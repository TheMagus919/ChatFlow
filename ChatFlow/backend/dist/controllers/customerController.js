...
class CustomerController {
    ...
    async create(req, res) {
        try {
             ...
             */
            const customerStatus = status === undefined
                ? 'new'
                : status;
            if (typeof customerStatus !== 'string' ||
                !validStatuses.includes(customerStatus)) {
                return res.status(400).json({
    ...
    async update(req, res) {
        try {
            ...
            const customer = await this.customerService.update(userId, customerId, {
                status: status
            });
            if (!customer) {
                return res.status(404).json({
    ...
    async getOne(req, res) {
        try {
            ...
            const customer = await this.customerService.findById(userId, customerId);
            if (!customer) {
                return res.status(404).json({
                    error: 'Customer not found'
                });
    ...
    async delete(req, res) {
        try {
            ...
            const deleted = await this.customerService.delete(userId, customerId);
            if (!deleted) {
                return res.status(404).json({
                    error: 'Customer not found'
                });
    ...
    async assignTags(req, res) {
        try {
            ...
            const customer = await this.customerTagService
                .assignTagsToCustomer(customerId, String(userId), tagIds.map(String));
            return res.json(customer);
        }
        catch (error) {
    ...
    async getCustomerTags(req, res) {
        try {
            ...
            const tags = await this.customerTagService
                .findTagsByCustomer(customerId, String(userId));
