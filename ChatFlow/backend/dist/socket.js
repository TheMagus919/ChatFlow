...
const initSocket = (server) => {
    ...
    io.on('connection', (socket) => {
        ...
        // WHATSAPP
        // ==========================
        socket.on('whatsapp_message', async (message) => {
            try {
            ...
            `, [conversationId, userId]);
                if (!rows.length) {
                    console.warn(`⚠️ Usuario ${userId} intentó emitir mensaje de WhatsApp en conversación ${conversationId}`);
                    return;
                }
                socket.to(`conversation_${conversationId}`).emit('whatsapp_message', message);
            }
            catch (error) {
                console.error('Error en whatsapp_message:', error);
            }
        });
        // ==========================
        // INSTAGRAM
        // ==========================
        socket.on('instagram_message', async (message) => {
            try {
                const conversationId = Number(message?.conversationId);
                if (!Number.isInteger(conversationId) ||
                    conversationId <= 0) {
            ...
            `, [conversationId, userId]);
                if (!rows.length) {
                    console.warn(`⚠️ Usuario ${userId} intentó emitir mensaje de Instagram en conversación ${conversationId}`);
                    return;
                }
                socket.to(`conversation_${conversationId}`).emit('instagram_message', message);
            }
            catch (error) {
                console.error('Error en instagram_message:', error);
            }
        });
        // ==========================
        // FACEBOOK
        // ==========================
        socket.on('facebook_message', async (message) => {
            try {
                const conversationId = Number(message?.conversationId);
                if (!Number.isInteger(conversationId) ||
                    conversationId <= 0) {
            ...
            `, [conversationId, userId]);
                if (!rows.length) {
                    console.warn(`⚠️ Usuario ${userId} intentó emitir mensaje de Facebook en conversación ${conversationId}`);
                    return;
                }
                socket.to(`conversation_${conversationId}`).emit('facebook_message', message);
            }
            catch (error) {
                console.error('Error en facebook_message:', error);
            }
        });
        // ==========================
        // DESCONEXIÓN
